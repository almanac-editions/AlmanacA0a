...
for this half is ours: transcript `own-habiro-le-even-hybrid-centers-explicit-torus` §2 and
...
(`corpus/CARD_ROUTING_2026-07-17.md`) has no `center-GL1` section, so this goal is served today only
...
The Librarian flagged — unprompted, twice — that its own `MINING_LOG.md` line covering these searches
...
(`lit-v2.jantzen-dilated-linkage-blocks`) but not as a source, and the many other Jantzen mentions
