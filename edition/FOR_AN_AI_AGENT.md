...
AlmanacA0a/
  ...
  ANCESTRY.md AUTHORSHIP.md EMBEDDING_NOTE.md MEASUREMENT_LOG.md
  ...
  source/
    ...
    lean/                 the Lean modules — A READING COPY; rebuilds only in the full development
...
### 2.5 The Lean layer — read-only here

`source/lean/` is **a reading copy**. Its `import` lines point into the larger development it was
extracted from, and it rebuilds only there; do not report that as a defect of the almanac, and
do not "fix" the imports. What you can do:
