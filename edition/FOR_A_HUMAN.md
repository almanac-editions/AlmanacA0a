...
> **The Lean sources in `source/lean/` are a copy for reading.** They were extracted from a larger
> development, and their `import` lines point back into it.
> You can read them, and you can check the recorded axioms against the concordance. To *rebuild*
> them you need the full project they came from; `source/record/formal/SOURCE_MANIFEST.json`
> identifies it, and `source/record/formal/RELEASE_20260721_source_all_clean.log` is the
...
**What you should see:** exactly `[propext, Classical.choice, Quot.sound]`.
...
| a Lean `import` cannot be found | expected: `source/lean/` is a reading copy; rebuilding needs the full development (see 2.2) |
