...
<span class="stamp warn">awaiting deposit</span>
<span class="stamp warn">pre-publication proposal</span>
<span class="stamp warn">rows freeze at deposit</span>
</p>

<div class="note warn">
<strong>This is a proposal; it becomes a publication only at deposit and outward
authorisation.</strong> It has been accepted by the record's owner as a faithful account of a closed
...
for r in rows:
    ...
    if r.get("axioms"):
        A(f"<dt>axioms</dt><dd><code>{esc(', '.join(r['axioms']))}</code></dd>")
...
only when its axiom base is exactly <code>propext, Classical.choice, Quot.sound</code>. Per-row
