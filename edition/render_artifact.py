...
def artifact_index():
    ...
    for key, title, blurb in GROUP_ORDER:
        ...
        for a in items:
            ...
            h = (f'{esc(a["sha256"])[:12]}…' if a.get("sha256")
...
A(f'<div class="grade g-k"><div class="n">{n_k}<span class="of"> / {len(rows)}</span></div>'
  ...
  "entry's claim, and its axiom base is exactly <code>propext, Classical.choice, Quot.sound</code> — "
...
A("<p><strong>Certification evidence.</strong> The closure signature of 2026-07-21 records thirteen "
  ...
  "server, and each returned the axiom base <code>propext, Classical.choice, Quot.sound</code>.</p>")
